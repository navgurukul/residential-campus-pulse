      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg border border-blue-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-blue-700">Most Active Resolver</p>
                <p className="text-lg font-bold text-blue-900">
                  {resolvers.reduce((prev, current) => 
                    (prev.totalEvaluations > current.totalEvaluations) ? prev : current
                  ).name}
                </p>
          ...
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-lg border border-green-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-green-700">Highest Avg Score</p>
                <p className="text-lg font-bold text-green-900">
                  {Math.max(...resolvers.map(r => r.averageScoreGiven)).toFixed(1)}
                </p>
                <p className="text-xs text-green-600">
                  by {resolvers.find(r => r.averageScoreGiven === Math.max(...resolvers.map(r => r.averageScoreGiven)))?.name}
                </p>
              </div>
          ...
          <div className="bg-gradient-to-r from-purple-50 to-violet-50 p-4 rounded-lg border border-purple-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-purple-700">Coverage Rate</p>
                <p className="text-lg font-bold text-purple-900">
                  {((resolvers.reduce((sum, r) => sum + r.campusesEvaluated, 0) / (resolvers.length * 9)) * 100).toFixed(1)}%
                </p>
                <p className="text-xs text-purple-600">Campus evaluation coverage</p>
              </div>
          ...
          <div className="bg-gradient-to-r from-orange-50 to-amber-50 p-4 rounded-lg border border-orange-200">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-orange-700">Evaluation Frequency</p>
                <p className="text-lg font-bold text-orange-900">
                  {(resolvers.reduce((sum, r) => sum + r.totalEvaluations, 0) / resolvers.length).toFixed(1)}
                </p>
                <p className="text-xs text-orange-600">Avg evaluations per resolver</p>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          <div className="bg-gray-50 p-4 rounded-lg">
            <h4 className="text-lg font-semibold text-gray-800 mb-4">Resolver Performance Distribution</h4>
            <div className="space-y-3">
              {['Excellent (6.0-7.0)', 'Good (5.0-5.9)', 'Average (4.0-4.9)', 'Below Average (3.0-3.9)', 'Poor (0-2.9)'].map((range, index) => {
                const [label, scoreRange] = range.split(' (');
                const [min, max] = scoreRange.replace(')', '').split('-').map(Number);
                const count = resolvers.filter(r => r.averageScoreGiven >= min && r.averageScoreGiven <= max).length;
                const percentage = (count / resolvers.length) * 100;
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <h4 className="text-lg font-semibold text-gray-800 mb-4">Activity Insights</h4>
            <div className="space-y-4">
              <div className="flex justify-between items-center p-3 bg-white rounded-lg">
                <span className="text-sm font-medium text-gray-700">Total Evaluations Completed</span>
                <span className="text-lg font-bold text-blue-600">
                  {resolvers.reduce((sum, r) => sum + r.totalEvaluations, 0)}
                </span>
              </div>
              <div className="flex justify-between items-center p-3 bg-white rounded-lg">
                <span className="text-sm font-medium text-gray-700">Average Score Across All Resolvers</span>
                <span className="text-lg font-bold text-green-600">
                  {(resolvers.reduce((sum, r) => sum + r.averageScoreGiven, 0) / resolvers.length).toFixed(2)}
                </span>
              </div>
              <div className="flex justify-between items-center p-3 bg-white rounded-lg">
                <span className="text-sm font-medium text-gray-700">Most Recent Activity</span>
                <span className="text-sm font-medium text-gray-900">
                  {new Date(Math.max(...resolvers.map(r => new Date(r.lastActivity).getTime()))).toLocaleDateString()}
                </span>
              </div>
              <div className="flex justify-between items-center p-3 bg-white rounded-lg">
                <span className="text-sm font-medium text-gray-700">Resolvers with 100% Campus Coverage</span>
                <span className="text-lg font-bold text-purple-600">
                  {resolvers.filter(r => r.campusesEvaluated === 9).length}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white p-3 rounded-lg">
              <h5 className="font-medium text-gray-800 mb-2">🎯 Improve Coverage</h5>
              <p className="text-sm text-gray-600">
                {resolvers.filter(r => r.campusesEvaluated < 5).length} resolvers have evaluated fewer than 5 campuses. 
                Consider encouraging broader campus evaluation participation.
              </p>
            </div>
            <div className="bg-white p-3 rounded-lg">
              <h5 className="font-medium text-gray-800 mb-2">📊 Score Consistency</h5>
              <p className="text-sm text-gray-600">
                Score range: {Math.min(...resolvers.map(r => r.averageScoreGiven)).toFixed(1)} - {Math.max(...resolvers.map(r => r.averageScoreGiven)).toFixed(1)}. 
                Consider calibration sessions for more consistent scoring.
              </p>
            </div>
          </div>
        </div>
